...
class FreeTrialController extends Controller {
    ...
    public function __construct(FreeTrial $freeTrial) { 
        ...
        View::share('viewPage', 'FreeTrial');
        View::share('sub_page_title', 'FreeTrial');
        View::share('helper',new Helper);
        View::share('heading','FreeTrial');
        View::share('route_url',route('freeTrial')); 
    ...
    public function index(FreeTrial $freeTrial, Request $request) 
    { 
        
        $page_title = 'FreeTrial'; 
        $page_action = 'View FreeTrial'; 
        ...
        if ((isset($search) && !empty($search))) {
            ...
            $freeTrial = FreeTrial::where(function($query) use($search) {
                        if (!empty($search)) {
        ...
        if($export=='pdf')
        {
           $pdf = PDF::loadView('packages::freeTrial.pdf', compact('corporateProfile', 'page_title', 'page_action','freeTrial'));
           return ($pdf->download('all-contacts.pdf'));
        }
         
        
        return view('packages::freeTrial.index', compact('result_set','freeTrial','data', 'page_title', 'page_action','sub_page_title'));
    }
